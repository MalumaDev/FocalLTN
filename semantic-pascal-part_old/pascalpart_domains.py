...
def get_pascalpart_data(
    ...
    labeled_ratio = labeled_ratio if labeled_ratio is not None else config["labeled_ratio"]
...
def data_to_domains(
    ...
    minibatch_size = minibatch_size if minibatch_size is not None else config["minibatch_size"]
    shuffle_buffer_size = shuffle_buffer_size if shuffle_buffer_size is not None else config["shuffle_buffer_size"]
    ...
    x1_isnotpart = ltnw.Domain("x1_isnotpart", [x1x2[0] for x1x2 in data.x1x2_isnotpart],
                               dataset_params=ds_params_with_map)
    x2_isnotpart = ltnw.Domain("x2_isnotpart", [x1x2[1] for x1x2 in data.x1x2_isnotpart],
                               dataset_params=ds_params_with_map)
