...
def get_theory(
        ...
        log_every_n_step: int = None,
        op_config: str = None,
):
    log_every_n_step = log_every_n_step if log_every_n_step is not None else config["training_steps_per_epoch"]
    op_config = op_config if op_config is not None else config["ltn_config"]
    if op_config == "stable_rl":
        operator_config = get_stable_rl_operator_config()
        formula_aggregator = ltn.Wrapper_Formula_Aggregator(
            ltn.fuzzy_ops.Aggreg_pMeanError(p=config["p_universal_quantifier"]))
...
def get_groundtruth_constraints(
    ...
    constraints.append(Rule_IsPartOfGroundtruth("is_partof_groundtruth", grounding, operator_config,
                                                doms_feed_dict={"x1": pascal_doms.x1_ispart,
                                                                "x2": pascal_doms.x2_ispart}))
    constraints.append(Rule_NotPartOfGroundtruth("not_partof_groundtruth", grounding, operator_config,
                                                 doms_feed_dict={"x1": pascal_doms.x1_isnotpart,
                                                                 "x2": pascal_doms.x2_isnotpart}))
    return constraints
