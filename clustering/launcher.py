...
for seed in range(5):  # range(5):
    for k in [1, -1]:
        for j in [1., 0.75, .5, .25]:
            ...
            """
            os.system(
                f"python script-stableproduct-ltn.py --csv-path results/stable_product_p2_{j * k}_forall_{seed}.csv"
                f" --p 2 --seed {seed} --imbalance {j * k} --forall_only")

            # Stable Product p=6
            os.system(
                f"python script-stableproduct-ltn.py --csv-path results/stable_product_p6_{j * k}_forall_{seed}.csv"
                f" --p 6 --seed {seed} --imbalance {j * k} --forall_only")

            # Stable Product p=2 with Focal Loss
            os.system(
                f"python script-stableproduct-ltn.py --csv-path results/stable_product_p2_{j * k}_forall_{seed}_focal.csv"
                f" --p 2 --seed {seed} --imbalance {j * k} --use_focal --forall_only")
